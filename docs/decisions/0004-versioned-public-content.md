...
The section editor initially presents ordered cards with move-up and move-down controls. Rich text prefers [PrimeVue Editor](https://primevue.dev/editor/) with its [Quill](https://github.com/slab/quill/blob/main/LICENSE) dependency and a restricted toolbar. Both dependencies must remain usable under no-fee open-source licenses. The selected release must pass dependency auditing and must not carry an unresolved HTML-export vulnerability; use a compatible unaffected release or another no-fee open-source editor when necessary. Server-side sanitization is required regardless of editor behavior.
