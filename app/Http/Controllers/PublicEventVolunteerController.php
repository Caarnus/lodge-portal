...
class PublicEventVolunteerController extends Controller
{
    public function store(Request $request, Lodge $lodge, EventOccurrence $occurrence, VolunteerCommitmentService $service, VolunteerEligibility $eligibility)
    {
        ...
        $service->commit($occurrence, $position, $request->user(), $request->user()); return back();
    }
    ...
    { abort_unless($occurrence->lodge_id === $lodge->id && $commitment->event_occurrence_id === $occurrence->id && $commitment->lodge_id === $lodge->id, 404); $service->withdraw($commitment, $request->user()); return back(); }
}
