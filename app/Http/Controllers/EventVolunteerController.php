...
class EventVolunteerController extends Controller
{
    ...
    {
        $this->authorize($request, $lodge, $event, $occurrence); $data = $request->validate(['position_id' => ['required', 'integer'], 'person_id' => ['required', 'integer']]);
        ...
        $person = Person::query()->whereKey($data['person_id'])->firstOrFail(); $target = $person->user; abort_unless($target && $eligibility->canVolunteer($target, $event), 422);
        $service->commit($occurrence, $position, $target, $request->user()); return back();
    }
    ...
    { $this->authorize($request, $lodge, $event, $occurrence); abort_unless($commitment->event_occurrence_id === $occurrence->id && $commitment->event_id === $event->id && $commitment->lodge_id === $lodge->id, 404); $service->withdraw($commitment, $request->user(), true); return back(); }
    private function authorize(Request $request, Lodge $lodge, Event $event, EventOccurrence $occurrence): void { abort_unless($event->lodge_id === $lodge->id && $occurrence->event_id === $event->id && $occurrence->lodge_id === $lodge->id, 404); abort_unless($request->user()?->hasLodgePermission($lodge, 'events.manage'), 403); }
}
