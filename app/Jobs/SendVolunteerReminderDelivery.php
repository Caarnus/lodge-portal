...
class SendVolunteerReminderDelivery implements ShouldQueue
{
    ...
    public function handle(VolunteerEligibility $eligibility): void
    {
        ...
        if (! $delivery || $delivery->status !== VolunteerReminderDeliveryStatus::Claimed || $delivery->attempted_at) return;
        $commitment = $delivery->commitment; $user = $commitment?->user;
        ...
        if (! $valid) { $delivery->update(['status' => VolunteerReminderDeliveryStatus::Skipped, 'skip_reason' => 'account_unavailable', 'skipped_at' => now()]); return; }
        if (EventVolunteerReminderDelivery::query()->whereKey($delivery->id)->where('status', VolunteerReminderDeliveryStatus::Claimed)->whereNull('attempted_at')->update(['attempted_at' => now()]) !== 1) return;
        try { Notification::route('mail', $user->email)->notify(new VolunteerStaffingReminder($delivery)); $delivery->update(['status' => VolunteerReminderDeliveryStatus::Sent, 'sent_at' => now(), 'last_error' => null]); } catch (\Throwable $exception) { $delivery->update(['status' => VolunteerReminderDeliveryStatus::Failed, 'failed_at' => now(), 'last_error' => str($exception->getMessage())->limit(1000)]); }
    }
}
