...
class RruleRecurrenceExpander implements RecurrenceExpander
{
    ...
    {
        return $this->rule($rule, $startsAt, $timeZone)->rfcString(false);
    }
    ...
    private function rule(string $rule, CarbonImmutable $startsAt, string $timeZone): RRule
    {
        ...
        if (! str_starts_with($normalized, 'RRULE:')) {
            $normalized = 'RRULE:'.$normalized;
        }

        try {
            return new RRule($normalized, $startsAt->setTimezone(new DateTimeZone($timeZone))->toDateTime());
