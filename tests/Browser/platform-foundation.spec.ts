...
test("registration requires a home lodge and reaches pending approval", async ({
    page,
}) => {
    ...
    await expect(
        page.getByRole("heading", { name: "Registration pending" }),
    ).toBeVisible();
...
test("platform and lodge administrators complete the two-lodge isolation flow", async ({
    page,
    context,
}) => {
    ...
    test.skip(
        ...
        "Run through the Docker browser service for the seeded administrator flow.",
    );
    ...
    const assignAdmin = async (email: string) => {
        ...
        await page
            .getByRole("button", { name: "Assign or create administrator" })
            .click();
    ...
    await expect(
        page.getByRole("link", { name: "Platform lodges" }),
    ).toHaveAttribute("href", "/platform/lodges");
    await expect(
        page.getByRole("link", { name: "Registrations" }),
    ...
    await expect(
        page.getByRole("link", { name: "Create lodge" }),
    ).toBeVisible();
    await expect(
    ...
    const documentHeightBeforeTooltip = await page.evaluate(
    ...
    await expect
        ...
        .toBe(documentHeightBeforeTooltip);
    ...
    const lodgeAId = await createLodge(
        lodgeAName,
        `A-${suffix}`,
        `browser-a-${suffix}`,
    );
    ...
    const lodgeBId = await createLodge(
        lodgeBName,
        `B-${suffix}`,
        `browser-b-${suffix}`,
    );
    ...
    const publishTemplateHome = async (
        lodgeId: string,
        lodgeSlug: string,
        lodgeName: string,
    ) => {
        ...
        await page
            .getByRole("button", { name: "Apply default template" })
            .click();
        const homeRow = page
            .locator("article")
            .filter({ hasText: /^Home/ })
        ...
        await expect(
            page.getByRole("heading", { name: lodgeName, exact: true }).first(),
        ).toBeVisible();
    ...
    const expandableLodgeName = page
        .getByRole("button", { name: "Expand lodge name" })
        .first();
    await expect(expandableLodgeName).toBeVisible();
    await expandableLodgeName.click();
    await expect(
        page.getByRole("button", { name: "Collapse lodge name" }).first(),
    ...
    await page.goto(`/lodges/${lodgeAId}/people`);
    ...
    await page
        .getByRole("button", { name: "Create person and membership" })
        .click();
    await expect(page.getByRole("dialog")).toHaveCount(0);
    await page
        .getByRole("button", { name: `View Hiram Browser ${suffix}` })
        .click();
    await page.getByRole("button", { name: "Edit" }).click();
    await page
        .getByRole("button", { name: "Create a new non-member relative" })
        .click();
    ...
    await page
        ...
        .selectOption({ label: "Spouse" });
    await page
        .getByRole("button", { name: "Add person and relationship" })
        .click();
    await expect(
        page.getByText(
            new RegExp(
                `Hiram Browser ${suffix} is spouse of Alex Browser ${suffix}`,
                "i",
            ),
        ),
    ).toBeVisible();
    ...
    const publishedOfficersRow = page
        .locator("article")
        .filter({ hasText: /^Officers/ })
        .first();
    const publicOfficersPage = publishedOfficersRow.getByRole("link", {
    ...
    await expect(
        page.getByRole("heading", { name: `Hiram Browser ${suffix}` }),
    ).toBeVisible();
    await expect(page.getByText(officerEmail)).toHaveCount(0);

    expect((await page.goto(`/lodges/${lodgeBId}/settings`))?.status()).toBe(
        403,
    );
    ...
    expect((await page.goto(`/lodges/${lodgeAId}/settings`))?.status()).toBe(
        200,
    );
    expect((await page.goto(`/lodges/${lodgeBId}/settings`))?.status()).toBe(
        200,
