...
const submit = () =>
    isNew.value
        ? form.post(`/lodges/${props.lodge.id}/events`)
        : form.put(`/lodges/${props.lodge.id}/events/${props.event.id}`);
...
<template>
    <main class="mx-auto max-w-4xl space-y-6 p-6">
        ...
        <form class="space-y-6" @submit.prevent="submit">
            ...
            <section class="grid gap-4 rounded-lg border p-5 md:grid-cols-2">
                <label class="md:col-span-2"
                    >Title<input
                        ...
                        required
                    />
                    <InputError :message="form.errors.title" /> </label
                ><label
                    >Slug<input
                        v-model="form.slug"
                        class="mt-1 w-full rounded-md border bg-background p-2"
                        required
                    />
                    <InputError :message="form.errors.slug" /> </label
                ><label
                    >Category<select
                ...
                ><label class="md:col-span-2"
                    >Description<textarea
                        ...
                        class="mt-1 min-h-28 w-full rounded-md border bg-background p-2"
                    /></label
                ><label
                    >Location<input
            ...
            <section class="grid gap-4 rounded-lg border p-5 md:grid-cols-2">
                ...
                <label
                    v-if="!isNew && form.errors.confirm_schedule_change"
                    class="md:col-span-2 flex items-start gap-2 rounded-md border border-amber-500/40 bg-amber-50 p-3 text-sm"
                    ><input
                        v-model="form.confirm_schedule_change"
                        type="checkbox"
                        class="mt-1"
                    /><span
                        >I understand this changes future occurrences. Preserve
                        protected occurrences and save this schedule
                        change.</span
                    ></label
                >
            </section>
            <section class="grid gap-4 rounded-lg border p-5 md:grid-cols-2">
                <h2 class="md:col-span-2 text-lg font-medium">
                    Audience and reservations
                </h2>
                <label
                    >Visibility<select
                ...
                ><label v-if="form.visibility !== 'public'"
                ...
                ><label class="flex items-center gap-2"
                    ><input
                        ...
                        type="checkbox"
                    />
                    Enable reservations</label
                ><label class="flex items-center gap-2"
                    ><input
                        v-model="form.guest_reservations_enabled"
                        type="checkbox"
                    />
                    Allow guest reservations</label
                ><label
                    >Capacity<input
                ...
                ><label class="flex items-center gap-2"
                    ><input
                        ...
                        type="checkbox"
                    />
                    Allow cross-lodge reservations</label
                >
            </section>
            ...
            <section v-if="!isNew" class="space-y-4 rounded-lg border p-5">
