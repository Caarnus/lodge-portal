        ...
        <section class="rounded border p-5">
            ...
            >
                ...
                <form
                    ...
                    @submit.prevent="
                        formFor(request).post(
                            `/lodges/${lodge.id}/newsletter-recipients/requests/${request.id}/approve`,
                        )
                    "
                >
                    <select
                ...
                <button
                    ...
                    @click="
                        useForm({}).post(
                            `/lodges/${lodge.id}/newsletter-recipients/requests/${request.id}/reject`,
                        )
                    "
                >
                    Reject
        ...
        <section class="rounded border p-5">
            ...
            <ul v-else class="mt-3 divide-y">
                <li
                    ...
                    class="py-3"
                >
                    <strong>{{ subscription.recipient.display_name }}</strong> —
                    sponsor: {{ subscription.sponsor.display_name }} ·
                    {{ subscription.status }}
                </li>
            </ul>
        </section>
