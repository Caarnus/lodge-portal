...
<template>
    <Head :title="`${lodge.name} newsletters`" />
    <main class="mx-auto max-w-5xl space-y-7 p-6">
        <header>
            <h1 class="text-3xl font-bold">Newsletters</h1>
            <p class="text-slate-600">Drafts stay private until published.</p>
        </header>
        <section class="divide-y rounded border">
            <article
            ...
            >
                <div class="min-w-0 flex-1">
                    ...
                    <p class="text-sm text-slate-500">
                        /{{ issue.slug }} · {{ issue.draft ? "Draft" : ""
                        }}{{ issue.draft && issue.published ? " + " : ""
                        }}{{ issue.published ? "Published" : "" }}
                    </p>
                </div>
                <Link
                    :href="`/lodges/${lodge.id}/newsletters/manage/${issue.id}/edit`"
                    class="underline"
                    >Edit</Link
                ><button
                    v-if="canPublish && issue.published"
                    class="underline"
                    @click="
                        router.post(
                            `/lodges/${lodge.id}/newsletters/manage/${issue.id}/unpublish`,
                        )
                    "
                >
                    Unpublish</button
                ><button class="text-red-700 underline" @click="remove(issue)">
                    Delete
                </button>
            </article>
            <p v-if="!issues.length" class="p-5 text-slate-600">
        ...
        <section class="rounded border p-5">
            ...
            <form
                ...
                @submit.prevent="
                    form.post(`/lodges/${lodge.id}/newsletters/manage`)
                "
            >
                <label
                    >Title<input
